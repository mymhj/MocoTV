...
/** 线程退出标志 */   
bool CFuSerialPort::s_bExit = false;
...
CFuSerialPort::CFuSerialPort(void *param)
...
CFuSerialPort::~CFuSerialPort(void)
{  
    CloseListenTread();  
    ClosePort();  
...
bool CFuSerialPort::InitPort(CString portNo /*= 1*/, UINT baud /*= CBR_9600*/, char parity /*= 'N'*/,
...
bool CFuSerialPort::InitPort(CString portNo, const LPDCB& plDCB)
{  
...
bool CFuSerialPort::openPort(CString portNo)
{  
    /** 进入临界段 */   
    //EnterCriticalSection(&m_csCommunicationSync);  
...
bool CFuSerialPort::OpenListenThread()
{  
    /** 检测线程是否已经开启了 */   
...
bool CFuSerialPort::CloseListenTread()
{     
    if (m_hListenThread != INVALID_HANDLE_VALUE)  
    {  
        /** 通知线程退出 */   
...
UINT CFuSerialPort::GetBytesInCOM()
{  
    DWORD dwError = 0;  /** 错误码 */   
...
UINT WINAPI CFuSerialPort::ListenThread(void* pParam)
{  
 /*   / ** 得到本类的指针 * /   
...
bool CFuSerialPort::ReadChar(unsigned char &cRecved)
{  
    BOOL  bResult     = TRUE;  
    DWORD BytesRead   = 0;  
    if(m_hComm == INVALID_HANDLE_VALUE)  
...
bool CFuSerialPort::Read(BYTE cRecved[25], DWORD length)
{  
    BOOL  bResult     = TRUE;  
    DWORD BytesRead   = 0;  
    if(m_hComm == INVALID_HANDLE_VALUE)  
...
bool CFuSerialPort::ReadSyn(BYTE cRecved[25], DWORD length)
{
	BOOL  bResult = TRUE;
	DWORD BytesRead = 0;
	if (m_hComm == INVALID_HANDLE_VALUE)
...
bool CFuSerialPort::Write(unsigned char* pData, unsigned int length)
{  
    BOOL   bResult     = TRUE;  
    DWORD  BytesToSend = 0;  
    if(m_hComm == INVALID_HANDLE_VALUE)  
